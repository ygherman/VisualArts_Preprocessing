...
def check_custom04_file(file_path):
    ...
    if file_path.exists():
        logger.debug(
            "File already exists. Deleting and creating new file."
        )
        os.remove(file_path)
...
def create_907_json(df_collection, collection_id, digitization_path):
    ...
    collection_907_dict = {k: rosetta_dict[k] for k in df_collection.index}

    if not Path(digitization_path/ "ROS" / (collection_id + "_907.json")).exists():
        with open(digitization_path/ "ROS" / (collection_id + "_907.json"), 'w') as fp:
...
def main(**collection):
    ...
    if type(collection) == VC_collections.Collection.Collection:
        ...
        branch = "VC-" + input("Please enter the name of the Branch (Architect, Design, Dance, Theater): : ")
    ...
    rosetta_file_path = minidom.parse(FileIO(FILE_PATH))
    logger.info(
    ...
    mask = (df['093c'].str.contains(collection_id)) | (df['093d'].str.contains(collection_id))
    ...
    file_full_path = aleph_custom04_path / (
        collection_id + "_alma_sysno.xlsx"
    )
